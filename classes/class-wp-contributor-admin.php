 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'Wp_Contributor_Admin' ) ) {
	/**
	...
	class Wp_Contributor_Admin {
		...
		function metabox_render_html( $post ) {
			...
			$selected_users = wpc_loader()->utils->get_contributors_list( $post->ID );
			...
			foreach ( $users as $user_id => $user_data ) {
				...
				if ( in_array( (string) $user_id, $selected_users[ $user_id ], true ) ) {
					$checked = 'checked';
				}
			...
			foreach ( $wp_users as $index => $user_data ) {
				if ( in_array( $user_data->ID, $selected_contributor, true ) ) {
					...
					$contributor_data[ $user_data->ID ] = array(
						...
						'display_name'    => $user_info->display_name ? $user_info->display_name : '',
						'user_nicename'   => $user_info->user_nicename ? $user_info->user_nicename : '',
						'user_avatar_url' => $user_profile_pic ? $user_profile_pic : '',
					);
				}
			}
	...
	/**
	 *  Prepare if class 'Wp_Contributor_Admin' exist.
